...
async def research_agent(query: str, model_name: str = "gpt-5-mini",
    ...
    """
    Build research_agent system prompt from registry and call the chat model with the query.
    ...
    # Get research agent memory context if available
    research_memory_context = ""
    chat_history_context = ""
    if session_context:
        research_memory = await session_context.get_agent_memory("research_agent")
        research_memory_context = await research_memory.get_context_string()
        ...
        if session_context.chat_id:
            ...
            if chat_messages:
                ...
                for msg in chat_messages[-10:]:  # Last 10 messages
                    ...
                    elif role == "assistant" and agent == "research_agent":
                        chat_history_parts.append(f"Assistant (research_agent): {content}")
        ...
        memory_metadata = {"timestamp": None, "query_type": "research"}
    ...
    system_prompt = build_system_prompt("research_agent", str(registry_path),
    ...
    print("=== Research Agent System Prompt ===")
    print(system_prompt)
    print("=== End System Prompt ===")

    # First call to the model to determine if tool is required
    if session_context:
        await session_context.send_nano("research_agent", "thinking…")
    ...
    if session_context:
        await session_context.send_nano("research_agent", "parsed response")
    ...
    print("=== Initial research_agent response ===")
    print(normalized)
    ...
    try:
        ...
        while True:
            ...
            if iteration >= max_iterations:
                warning_msg = f"Max iterations ({max_iterations}) reached in research_agent; returning best-effort response."
                print(warning_msg)
                if session_context:
                    await session_context.send_nano("research_agent", "Max iterations reached: showing last message")
                return {"text": str(last_normalized)}
            ...
            if session_context:
                await session_context.send_nano("research_agent", "Processing tool result")
            ...
            print("=== Iteration research_agent response ===")
